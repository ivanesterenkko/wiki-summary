...
class SummaryService:
    async def generate_and_save(self, db: AsyncSession, article: Article):
        try:
            existing_summary = await crud_summary.get_by_article_id(db, article.id)
            if existing_summary:
                return existing_summary

            summary = await gpt_summarize(article.content)
            if not summary:
                raise RuntimeError("Сервис генерации summary вернул пустой результат")
            ...
            raise HTTPException(
                ...
                detail=f"Ошибка при генерации summary: {str(ex)}"
            ) from ex
    ...
    async def get_summary(self, db: AsyncSession, url: str):
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Ошибка при получении summary: {str(ex)}"
            ) from ex